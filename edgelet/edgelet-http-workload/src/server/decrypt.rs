...
impl DecryptHandler {
    pub fn new(key_connector: Connector, identity_client: IdentityClient) -> Self {
        ...
        DecryptHandler { key_store, identity_client: Arc::new(Mutex::new(identity_client)) }
    }
}
    ...
    fn handle(
        ...
        let response = params
            ...
            .and_then(move |(id, request)| -> Result<_, Error> {
                ...
                let ciphertext = fut.and_then(|k| { get_plaintext(key_store, k, initialization_vector, plaintext)})
